...
**Built-in dispatch.** `CALL` carries a BUILTIN integer ID. The handler switches on this ID to call the appropriate implementation (string methods, type conversions, timestamp/duration accessors, math functions). No string lookup at runtime. String extensions include `charAt` (codepoint-aware), `lastIndexOf`, and `strings.quote`. Math extensions include `math.greatest` and `math.least` (variable-arity, support 1-N args or a single list arg). Namespace-qualified functions (`math.*`, `strings.*`) are resolved in the compiler via dedicated lookup tables.
